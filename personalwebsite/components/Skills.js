...
const Skills = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://192.168.1.5:3000/api/skill');
  ...
  <div>
   ...
   <div className=' font-primaryFont text-textColor pt-40 md:pt-56  '>
      <h1 className=' text-secondery text-center font-bold items-center md:text-xl lg:text-2xl text-mdtracking-widest '>SKILLS</h1>
              ...
              <div className=' w-11/12 pl-5 md:pl-0 md:w-4/12 '>
                   ...
                   <ul className='flex-col flex space-y-5'>
                     ...
                     {programmingData.map(item => (
                       <li onClick={() => toggleDropdown2(item.id)} className=' flex-1 text-start p-5 bg-boxShadow box-shadow-xl rounded-lg ' key={item.id}>
                         <div className='flex justify-between pb-5'>
                         <p>{item.lang}</p>
                          <button >
                            <FontAwesomeIcon
                             icon={ programToggle[item.id]? faChevronDown: faChevronUp } 
                             />
                             </button>
                         </div>
                         <ProgressBar className={`${programToggle[item.id]? ' block' : 'hidden' } h-10 pt-3 flex `}
                          height='5px'
                           width='90%'
                            bgColor='#BA1168'
               ...
               <div className=' w-11/12 pl-5 md:pl-0 md:w-4/12  '> 
                     ...
                     <ul className='flex-col flex space-y-5 '>
                       <h2 className='p-5   pt-20 md:pt-5 text-shadow-xl'>Cyber Security</h2>
                       {cyberSecurityData.map(item => (
                        
                       <li  onClick={() => toggleDropdown(item.id)} className='flex-1 text-start p-5  bg-boxShadow box-shadow-xl rounded-lg' key={item.id}>
                      <div className='flex justify-between pb-5'>
                      <p >{item.feild}</p>
                      <button>
                        <FontAwesomeIcon 
                        icon={ cyberSecurityToggle[item.id]? faChevronDown: faChevronUp } 
                        />
                        </button>
                      </div>
                           <ProgressBar 
